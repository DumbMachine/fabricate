#!/usr/bin/env bash
set -euo pipefail

mode=${1:-direct}
if [ "$mode" != "direct" ] && [ "$mode" != "proxy" ]; then
  echo "usage: $0 <direct|proxy>" >&2
  exit 2
fi

token=${FAB_PIPEDRIVE_TOKEN:-}
if [ -z "$token" ]; then
  echo "FAB_PIPEDRIVE_TOKEN is required" >&2
  exit 1
fi

if [ "$mode" = "direct" ]; then
  if [ -z "${FAB_PIPEDRIVE_URL:-}" ]; then
    echo "FAB_PIPEDRIVE_URL is required in direct mode" >&2
    exit 1
  fi
  base=${FAB_PIPEDRIVE_URL%/}
  curl_opts=(-sS --fail)
else
  base="https://api.pipedrive.com"
  curl_opts=(-sS --fail --proxy "${HTTPS_PROXY:?HTTPS_PROXY is required in proxy mode}")
  if [ -n "${SSL_CERT_FILE:-}" ]; then
    curl_opts+=(--cacert "$SSL_CERT_FILE")
  fi
fi

read_json() {
  curl "${curl_opts[@]}" -H "Authorization: Bearer $token" -H "Content-Type: application/json" "$@"
}

persons=$(read_json "$base/api/v2/persons")
echo "$persons" | jq -e '.. | strings | select(. == "dana@northwind.example")' >/dev/null

deal=$(read_json "$base/api/v2/deals/301")
echo "$deal" | jq -e '.. | strings | select(. == "INV-4812")' >/dev/null

updated=$(read_json -X PATCH "$base/api/v2/deals/301" -d '{"status":"won"}')
echo "$updated" | jq -e '.data.status == "won"' >/dev/null

created=$(read_json -X POST "$base/api/v2/persons" -d '{"name":"Casey Pike"}')
echo "$created" | jq -e '.data.name == "Casey Pike"' >/dev/null

python3 - "$mode" <<'PY'
import json, os, sys
from datetime import datetime, timezone

mode = sys.argv[1]
mode_report = {
    "messagesAfter": 10,
    "messagesBefore": 9,
    "operations": {"read": "passed", "write": "passed", "persistence": "passed"},
    "status": "passed",
}
path = os.environ.get("FAB_COMPATIBILITY_REPORT")
if path:
    report = {
        "api": "Pipedrive REST v2",
        "environment": {
            "label": "Acme Pipedrive",
            "manifest": "environments/acme-pipedrive.yaml",
            "messages": 9,
        },
        "integration": "pipedrive",
        "modes": {},
        "operationLabels": {"read": "Read pipeline", "write": "Win the INV-4812 deal", "persistence": "Confirm persistence"},
        "verification": {
            "kind": "curl",
            "label": "HTTP client",
            "client": os.environ.get("FAB_COMPATIBILITY_CLIENT", "curl"),
            "title": "HTTP API verification",
        },
    }
    try:
        with open(path, encoding="utf-8") as fh:
            report = json.load(fh)
    except FileNotFoundError:
        pass
    report["modes"][mode] = mode_report
    report["testedAt"] = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + "Z"
    report["testedCommit"] = os.environ.get("FAB_COMPATIBILITY_COMMIT", "unknown")
    client = os.environ.get("FAB_COMPATIBILITY_CLIENT")
    if client:
        report.setdefault("verification", {})["client"] = client
    with open(path, "w", encoding="utf-8") as fh:
        json.dump(report, fh, indent=2)
        fh.write("\n")
print(json.dumps({"mode": mode, **mode_report}))
PY
