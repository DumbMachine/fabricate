...
func TestAcmeScenarioReadWriteRead(t *testing.T) {
	...
	if listed.Code != http.StatusOK || !strings.Contains(listed.Body.String(), `"checkout_reliability"`) {
		t.Fatalf("list flags = %d %s", listed.Code, listed.Body.String())
	}
	...
	if got.Code != http.StatusOK || !strings.Contains(got.Body.String(), `"checkout_reliability"`) {
		t.Fatalf("get flag = %d %s", got.Code, got.Body.String())
	}
	...
	if created.Code != http.StatusOK || !strings.Contains(created.Body.String(), `"id":"posthog.flag-0001"`) {
		t.Fatalf("create = %d %s", created.Code, created.Body.String())
	}

	persisted := request(t, handler, http.MethodGet, "/api/projects/1/feature_flags/posthog.flag-0001/", "", testToken)
