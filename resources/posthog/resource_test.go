...
const testToken = "fab_posthog_test_token"
...
func openTestDB(t *testing.T, doc scenario.Document) *sql.DB {
	t.Helper()
	db, err := sql.Open("sqlite", "file:"+filepath.Join(t.TempDir(), "posthog.db")+"?_pragma=foreign_keys(1)&_pragma=busy_timeout(5000)")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAcmeScenarioValidatesAndRoundTrips(t *testing.T) {
	doc := loadScenario(t, "acme-flags.v1.json")
	codec := NewResource().Scenarios()
...
func TestCompiledOpenAPIContractIsValid(t *testing.T) {
	...
	if compiled.Index.Len() < 100 {
		t.Fatalf("compiled operation count = %d, want the full PostHog catalog", compiled.Index.Len())
	}
...
func TestAcmeScenarioReadWriteRead(t *testing.T) {
	db := openTestDB(t, loadScenario(t, "acme-flags.v1.json"))
	handler := newTestHandler(t, db, &testIDs{})

	unauthorized := request(t, handler, http.MethodGet, "/api/projects/1/feature_flags/", "", "")
	if unauthorized.Code != http.StatusUnauthorized {
		t.Fatalf("unauthorized = %d %s", unauthorized.Code, unauthorized.Body.String())
	}

	listed := request(t, handler, http.MethodGet, "/api/projects/1/feature_flags/", "", testToken)
	...
	created := request(t, handler, http.MethodPost, "/api/projects/1/feature_flags/", `{"key":"billing_export","name":"Billing CSV export"}`, testToken)
	if created.Code != http.StatusOK && created.Code != http.StatusCreated {
		t.Fatalf("create = %d %s", created.Code, created.Body.String())
	}
	if !strings.Contains(created.Body.String(), `billing_export`) {
	...
	persisted := request(t, handler, http.MethodGet, strings.ReplaceAll("/api/projects/1/feature_flags/{id}/", "{id}", id), "", testToken)
	if persisted.Code != http.StatusOK || !strings.Contains(persisted.Body.String(), `billing_export`) {
