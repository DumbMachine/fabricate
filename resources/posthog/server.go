...
func (r jsonResponse) VisitFeatureFlagsListResponse(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitFeatureFlagsRetrieveResponse(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitFeatureFlagsCreateResponse(w http.ResponseWriter) error {
	return r.write(w)
}
...
func writeError(w http.ResponseWriter, status int, message string) {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(status)
	_, _ = w.Write([]byte(fmt.Sprintf(`{"type":"authentication_error","code":"invalid","detail":%q,"attr":null}`, message)))
}
...
func newServer(ctx context.Context, dependencies httpresource.ServerDependencies) (*server, error) {
	if dependencies.DB == nil || dependencies.Clock == nil || dependencies.IDs == nil || dependencies.Secrets == nil {
		return nil, fmt.Errorf("posthog: database, clock, ID generator, and secrets are required")
	}
	token, err := dependencies.Secrets.Get(ctx, "token")
	if err != nil {
		return nil, fmt.Errorf("posthog: load synthetic token: %w", err)
	}
	if token == "" {
		return nil, fmt.Errorf("posthog: synthetic token is empty")
	}
	spec, err := generated.GetSwagger()
	if err != nil {
		return nil, fmt.Errorf("posthog: load OpenAPI: %w", err)
	}
...
func (s *server) FeatureFlagsList(ctx context.Context, request generated.FeatureFlagsListRequestObject) (generated.FeatureFlagsListResponseObject, error) {
	if strings.TrimSpace(request.ProjectId) == "" {
		return jsonResponse{status: http.StatusBadRequest, body: map[string]any{
			"type": "invalid_request", "code": "invalid", "detail": "project_id is required", "attr": nil,
		}}, nil
	}
	items, err := listJSON(ctx, s.db, `SELECT body FROM flags ORDER BY id`)
	if err != nil {
		return nil, err
	}
...
func (s *server) FeatureFlagsRetrieve(ctx context.Context, request generated.FeatureFlagsRetrieveRequestObject) (generated.FeatureFlagsRetrieveResponseObject, error) {
	item, err := scanJSON(s.db.QueryRowContext(ctx, `SELECT body FROM flags WHERE id=?`, request.Id))
	if errors.Is(err, sql.ErrNoRows) {
		return jsonResponse{status: http.StatusNotFound, body: map[string]any{
			"type": "invalid_request", "code": "not_found", "detail": "Not found.", "attr": nil,
		}}, nil
	}
	if err != nil {
		return nil, err
...
func (s *server) FeatureFlagsCreate(ctx context.Context, request generated.FeatureFlagsCreateRequestObject) (generated.FeatureFlagsCreateResponseObject, error) {
	item := bodyMap(ctx)
	if request.Body != nil {
	...
	if strings.TrimSpace(key) == "" {
		return jsonResponse{status: http.StatusBadRequest, body: map[string]any{
			"type": "validation_error", "code": "invalid_input", "detail": "key is required", "attr": "key",
		}}, nil
	}
	id, err := s.ids.Next(ctx, "posthog.flag")
	if err != nil {
		return nil, err
	}
	item["id"] = id
	...
	if _, err := s.db.ExecContext(ctx, `INSERT INTO flags(id, body) VALUES(?, ?)`, id, string(raw)); err != nil {
		return nil, err
	}
	return jsonResponse{body: item}, nil
}
