...
import (
	...
	"github.com/dumbmachine/fabricate/resources/github/generated"
...
func (r jsonResponse) VisitListRepoIssuesResponse(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitGetIssueResponse(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitCreateIssueResponse(w http.ResponseWriter) error {
	return r.write(w)
}
...
func newServer(ctx context.Context, dependencies httpresource.ServerDependencies) (*server, error) {
	if dependencies.DB == nil || dependencies.Clock == nil || dependencies.IDs == nil || dependencies.Secrets == nil {
		return nil, fmt.Errorf("github: database, clock, ID generator, and secrets are required")
	}
	token, err := dependencies.Secrets.Get(ctx, "token")
	if err != nil {
		return nil, fmt.Errorf("github: load synthetic token: %w", err)
	}
	if token == "" {
		return nil, fmt.Errorf("github: synthetic token is empty")
	}
	spec, err := generated.GetSwagger()
	if err != nil {
		return nil, fmt.Errorf("github: load OpenAPI: %w", err)
	}
...
func (s *server) ListRepoIssues(ctx context.Context, _ generated.ListRepoIssuesRequestObject) (generated.ListRepoIssuesResponseObject, error) {
	items, err := listJSON(ctx, s.db, `SELECT body FROM issues ORDER BY id`)
	if err != nil {
		return nil, err
	}
	return jsonResponse{body: items}, nil
}

func (s *server) GetIssue(ctx context.Context, request generated.GetIssueRequestObject) (generated.GetIssueResponseObject, error) {
	item, err := scanJSON(s.db.QueryRowContext(ctx, `SELECT body FROM issues WHERE id=?`, request.IssueNumber))
...
func (s *server) CreateIssue(ctx context.Context, request generated.CreateIssueRequestObject) (generated.CreateIssueResponseObject, error) {
	item := bodyMap(ctx)
	if request.Body != nil {
		mergeMap(item, asMap(request.Body))
	}
	required, _ := item["title"].(string)
	if strings.TrimSpace(required) == "" {
		return jsonResponse{status: http.StatusBadRequest, body: map[string]any{"message": "title is required"}}, nil
	}
	id, err := s.ids.Next(ctx, "github.issue")
	if err != nil {
		return nil, err
	}
	item["id"] = id
	...
	if _, err := s.db.ExecContext(ctx, `INSERT INTO issues(id, body) VALUES(?, ?)`, id, string(raw)); err != nil {
		return nil, err
	}
	return jsonResponse{body: item}, nil
}
