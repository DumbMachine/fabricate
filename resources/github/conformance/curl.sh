...
token=${FAB_GITHUB_TOKEN:-}
if [ -z "$token" ]; then
  echo "FAB_GITHUB_TOKEN is required" >&2
  exit 1
fi

if [ "$mode" = "direct" ]; then
  if [ -z "${FAB_GITHUB_URL:-}" ]; then
    echo "FAB_GITHUB_URL is required in direct mode" >&2
    exit 1
  fi
  base=${FAB_GITHUB_URL%/}
  curl_opts=(-sS --fail)
...
listed=$(read_json "$base/repos/acme/web/issues")
echo "$listed" | jq -e '.. | strings | select(. == "Checkout reliability")' >/dev/null

got=$(read_json "$base/repos/acme/web/issues/1")
echo "$got" | jq -e '.. | strings | select(. == "Checkout reliability")' >/dev/null

created=$(read_json -X POST "$base/repos/acme/web/issues" -d '{"title":"CSV export downloads an empty file"}')
created_id=$(echo "$created" | jq -r '.id')
test -n "$created_id" && test "$created_id" != "null"

persisted=$(read_json "$base/repos/acme/web/issues/$created_id")
echo "$persisted" | jq -e '.. | strings | select(. == "CSV export downloads an empty file")' >/dev/null
...
if path:
    report = {
        ...
        "environment": {
            "label": "Acme GitHub",
            "manifest": "environments/acme-github.yaml",
            "messages": 3,
        },
        "integration": "github",
        "modes": {},
        "operationLabels": {"read": "List issues", "write": "Create an issue", "persistence": "Confirm persistence"},
        "verification": {
            "kind": "curl",
            "label": "HTTP client",
