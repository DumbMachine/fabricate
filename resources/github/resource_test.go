...
const testToken = "fab_github_test_token"
...
func openTestDB(t *testing.T, doc scenario.Document) *sql.DB {
	t.Helper()
	db, err := sql.Open("sqlite", "file:"+filepath.Join(t.TempDir(), "github.db")+"?_pragma=foreign_keys(1)&_pragma=busy_timeout(5000)")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAcmeScenarioValidatesAndRoundTrips(t *testing.T) {
	doc := loadScenario(t, "acme-issues.v1.json")
	codec := NewResource().Scenarios()
...
func TestCompiledOpenAPIContractIsValid(t *testing.T) {
	...
	if compiled.Index.Len() < 400 {
		t.Fatalf("compiled operation count = %d, want the full GitHub catalog", compiled.Index.Len())
	}
...
func TestAcmeScenarioReadWriteRead(t *testing.T) {
	db := openTestDB(t, loadScenario(t, "acme-issues.v1.json"))
	handler := newTestHandler(t, db, &testIDs{})

	unauthorized := request(t, handler, http.MethodGet, "/repos/acme/web/issues", "", "")
	if unauthorized.Code != http.StatusUnauthorized {
		t.Fatalf("unauthorized = %d %s", unauthorized.Code, unauthorized.Body.String())
	}

	listed := request(t, handler, http.MethodGet, "/repos/acme/web/issues", "", testToken)
	...
	created := request(t, handler, http.MethodPost, "/repos/acme/web/issues", `{"title":"CSV export downloads an empty file"}`, testToken)
	if created.Code != http.StatusOK && created.Code != http.StatusCreated {
		t.Fatalf("create = %d %s", created.Code, created.Body.String())
	}
	if !strings.Contains(created.Body.String(), `CSV export downloads an empty file`) {
	...
	persisted := request(t, handler, http.MethodGet, strings.ReplaceAll("/repos/acme/web/issues/{id}", "{id}", id), "", testToken)
	if persisted.Code != http.StatusOK || !strings.Contains(persisted.Body.String(), `CSV export downloads an empty file`) {
