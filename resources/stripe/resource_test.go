...
const testToken = "fab_stripe_test_token"
...
func openTestDB(t *testing.T, doc scenario.Document) *sql.DB {
	t.Helper()
	db, err := sql.Open("sqlite", "file:"+filepath.Join(t.TempDir(), "stripe.db")+"?_pragma=foreign_keys(1)&_pragma=busy_timeout(5000)")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAcmeScenarioValidatesAndRoundTrips(t *testing.T) {
	doc := loadScenario(t, "acme-customers.v1.json")
	codec := NewResource().Scenarios()
...
func TestCompiledOpenAPIContractIsValid(t *testing.T) {
	...
	if compiled.Index.Len() < 200 {
		t.Fatalf("compiled operation count = %d, want the full Stripe catalog", compiled.Index.Len())
	}
...
func TestAcmeScenarioReadWriteRead(t *testing.T) {
	db := openTestDB(t, loadScenario(t, "acme-customers.v1.json"))
	handler := newTestHandler(t, db, &testIDs{})

	unauthorized := request(t, handler, http.MethodGet, "/v1/customers", "", "")
	if unauthorized.Code != http.StatusUnauthorized {
		t.Fatalf("unauthorized = %d %s", unauthorized.Code, unauthorized.Body.String())
	}

	listed := request(t, handler, http.MethodGet, "/v1/customers", "", testToken)
	if listed.Code != http.StatusOK || !strings.Contains(listed.Body.String(), `Northwind`) {
		t.Fatalf("list = %d %s", listed.Code, listed.Body.String())
	}
	if !strings.Contains(listed.Body.String(), `Pied Piper`) {
		t.Fatalf("haystack missing Pied Piper: %s", listed.Body.String())
	}
	if strings.Count(listed.Body.String(), `"id":`) < 20 {
		t.Fatalf("customer haystack too small: %s", listed.Body.String())
	}

	got := request(t, handler, http.MethodGet, "/v1/customers/cus_northwind", "", testToken)
	if got.Code != http.StatusOK || !strings.Contains(got.Body.String(), `Northwind`) {
		t.Fatalf("get = %d %s", got.Code, got.Body.String())
	}

	created := request(t, handler, http.MethodPost, "/v1/customers", `{"name":"Acme Billing","email":"billing@acme.example"}`, testToken)
	if created.Code != http.StatusOK && created.Code != http.StatusCreated {
		t.Fatalf("create = %d %s", created.Code, created.Body.String())
	}
	if !strings.Contains(created.Body.String(), `Acme Billing`) {
	...
	persisted := request(t, handler, http.MethodGet, strings.ReplaceAll("/v1/customers/{id}", "{id}", id), "", testToken)
	if persisted.Code != http.StatusOK || !strings.Contains(persisted.Body.String(), `Acme Billing`) {
...
func TestAllOperationsAreRouted(t *testing.T) {
	...
	minimal := "acme-customers.v1.json"
