...
import (
	...
	"github.com/dumbmachine/fabricate/resources/stripe/generated"
...
func (r jsonResponse) VisitGetCustomersResponse(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitGetCustomersCustomerResponse(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitPostCustomersResponse(w http.ResponseWriter) error {
	return r.write(w)
}
...
func newServer(ctx context.Context, dependencies httpresource.ServerDependencies) (*server, error) {
	if dependencies.DB == nil || dependencies.Clock == nil || dependencies.IDs == nil || dependencies.Secrets == nil {
		return nil, fmt.Errorf("stripe: database, clock, ID generator, and secrets are required")
	}
	token, err := dependencies.Secrets.Get(ctx, "token")
	if err != nil {
		return nil, fmt.Errorf("stripe: load synthetic token: %w", err)
	}
	if token == "" {
		return nil, fmt.Errorf("stripe: synthetic token is empty")
	}
	spec, err := generated.GetSwagger()
	if err != nil {
		return nil, fmt.Errorf("stripe: load OpenAPI: %w", err)
	}
...
func (s *server) GetCustomers(ctx context.Context, _ generated.GetCustomersRequestObject) (generated.GetCustomersResponseObject, error) {
	items, err := listJSON(ctx, s.db, `SELECT body FROM customers ORDER BY id`)
	if err != nil {
		return nil, err
	}
...
func (s *server) GetCustomersCustomer(ctx context.Context, request generated.GetCustomersCustomerRequestObject) (generated.GetCustomersCustomerResponseObject, error) {
	item, err := scanJSON(s.db.QueryRowContext(ctx, `SELECT body FROM customers WHERE id=?`, request.Customer))
...
func (s *server) PostCustomers(ctx context.Context, request generated.PostCustomersRequestObject) (generated.PostCustomersResponseObject, error) {
	item := bodyMap(ctx)
	if request.Body != nil {
	...
	id, err := s.ids.Next(ctx, "stripe.customer")
	if err != nil {
		return nil, err
	}
	item["id"] = id
	...
	if _, err := s.db.ExecContext(ctx, `INSERT INTO customers(id, body) VALUES(?, ?)`, id, string(raw)); err != nil {
		return nil, err
	}
	return jsonResponse{body: item}, nil
}
