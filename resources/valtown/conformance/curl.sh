...
token=${FAB_VALTOWN_TOKEN:-}
if [ -z "$token" ]; then
  echo "FAB_VALTOWN_TOKEN is required" >&2
  exit 1
fi

if [ "$mode" = "direct" ]; then
  if [ -z "${FAB_VALTOWN_URL:-}" ]; then
    echo "FAB_VALTOWN_URL is required in direct mode" >&2
    exit 1
  fi
  base=${FAB_VALTOWN_URL%/}
  curl_opts=(-sS --fail)
else
...
listed=$(read_json "$base/v2/vals")
echo "$listed" | jq -e '.. | strings | select(. == "acme-webhooks")' >/dev/null

got=$(read_json "$base/v2/vals/val_webhooks")
echo "$got" | jq -e '.. | strings | select(. == "acme-webhooks")' >/dev/null

created=$(read_json -X POST "$base/v2/vals" -d '{"name":"billing-export","privacy":"unlisted"}')
created_id=$(echo "$created" | jq -r '.id')
test -n "$created_id" && test "$created_id" != "null"

persisted=$(read_json "$base/v2/vals/$created_id")
echo "$persisted" | jq -e '.. | strings | select(. == "billing-export")' >/dev/null
...
if path:
    report = {
        ...
        "environment": {
            "label": "Acme Val Town",
            "manifest": "environments/acme-valtown.yaml",
            "messages": 3,
        },
        "integration": "valtown",
        "modes": {},
        "operationLabels": {"read": "List vals", "write": "Create a val", "persistence": "Confirm persistence"},
        "verification": {
            "kind": "curl",
            "label": "HTTP client",
