...
func TestAcmeScenarioReadWriteRead(t *testing.T) {
	...
	created := request(t, handler, http.MethodPost, "/v2/vals", `{"name":"billing-export","privacy":"unlisted"}`, testToken)
	if created.Code != http.StatusOK && created.Code != http.StatusCreated {
		t.Fatalf("create status = %d %s", created.Code, created.Body.String())
	}
	if !strings.Contains(created.Body.String(), `"id":"valtown.val-0001"`) {
		t.Fatalf("create = %d %s", created.Code, created.Body.String())
	}

	persisted := request(t, handler, http.MethodGet, "/v2/vals/valtown.val-0001", "", testToken)
	if persisted.Code != http.StatusOK || !strings.Contains(persisted.Body.String(), `"billing-export"`) {
