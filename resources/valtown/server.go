...
import (
	...
	"github.com/dumbmachine/fabricate/resources/valtown/generated"
...
func (r jsonResponse) VisitValsList2Response(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitValsGet2Response(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitValsCreate2Response(w http.ResponseWriter) error {
	return r.write(w)
}
...
func newServer(ctx context.Context, dependencies httpresource.ServerDependencies) (*server, error) {
	if dependencies.DB == nil || dependencies.Clock == nil || dependencies.IDs == nil || dependencies.Secrets == nil {
		return nil, fmt.Errorf("valtown: database, clock, ID generator, and secrets are required")
	}
	token, err := dependencies.Secrets.Get(ctx, "token")
	if err != nil {
		return nil, fmt.Errorf("valtown: load synthetic token: %w", err)
	}
	if token == "" {
		return nil, fmt.Errorf("valtown: synthetic token is empty")
	}
	spec, err := generated.GetSwagger()
	if err != nil {
		return nil, fmt.Errorf("valtown: load OpenAPI: %w", err)
	}
...
func (s *server) ValsList2(ctx context.Context, _ generated.ValsList2RequestObject) (generated.ValsList2ResponseObject, error) {
	items, err := listJSON(ctx, s.db, `SELECT body FROM vals ORDER BY id`)
	if err != nil {
		return nil, err
	}
...
func (s *server) ValsGet2(ctx context.Context, request generated.ValsGet2RequestObject) (generated.ValsGet2ResponseObject, error) {
	item, err := scanJSON(s.db.QueryRowContext(ctx, `SELECT body FROM vals WHERE id=?`, request.ValId))
...
func (s *server) ValsCreate2(ctx context.Context, request generated.ValsCreate2RequestObject) (generated.ValsCreate2ResponseObject, error) {
	item := bodyMap(ctx)
	if request.Body != nil {
	...
	id, err := s.ids.Next(ctx, "valtown.val")
	if err != nil {
		return nil, err
	}
	item["id"] = id
	if _, ok := item["privacy"]; !ok {
	...
	if _, err := s.db.ExecContext(ctx, `INSERT INTO vals(id, body) VALUES(?, ?)`, id, string(raw)); err != nil {
		return nil, err
	}
	return jsonResponse{body: item}, nil
}
