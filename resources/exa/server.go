...
func (r jsonResponse) VisitListResearchTasksResponse(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitGetResearchTaskResponse(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitCreateResearchTaskResponse(w http.ResponseWriter) error {
	return r.write(w)
}
...
func newServer(ctx context.Context, dependencies httpresource.ServerDependencies) (*server, error) {
	if dependencies.DB == nil || dependencies.Clock == nil || dependencies.IDs == nil || dependencies.Secrets == nil {
		return nil, fmt.Errorf("exa: database, clock, ID generator, and secrets are required")
	}
	token, err := dependencies.Secrets.Get(ctx, "token")
	if err != nil {
		return nil, fmt.Errorf("exa: load synthetic token: %w", err)
	}
	if token == "" {
		return nil, fmt.Errorf("exa: synthetic token is empty")
	}
	spec, err := generated.GetSwagger()
	if err != nil {
		return nil, fmt.Errorf("exa: load OpenAPI: %w", err)
	}
...
func (s *server) ListResearchTasks(ctx context.Context, _ generated.ListResearchTasksRequestObject) (generated.ListResearchTasksResponseObject, error) {
	items, err := listJSON(ctx, s.db, `SELECT body FROM tasks ORDER BY id`)
	if err != nil {
		return nil, err
	}
...
func (s *server) GetResearchTask(ctx context.Context, request generated.GetResearchTaskRequestObject) (generated.GetResearchTaskResponseObject, error) {
	item, err := scanJSON(s.db.QueryRowContext(ctx, `SELECT body FROM tasks WHERE id=?`, request.Id))
...
func (s *server) CreateResearchTask(ctx context.Context, request generated.CreateResearchTaskRequestObject) (generated.CreateResearchTaskResponseObject, error) {
	item := bodyMap(ctx)
	if request.Body != nil {
		mergeMap(item, asMap(request.Body))
	}
	required, _ := item["instructions"].(string)
	if strings.TrimSpace(required) == "" {
		return jsonResponse{status: http.StatusBadRequest, body: map[string]any{"message": "instructions is required"}}, nil
	}
	id, err := s.ids.Next(ctx, "exa.task")
	if err != nil {
		return nil, err
	}
	item["id"] = id
	...
	if _, err := s.db.ExecContext(ctx, `INSERT INTO tasks(id, body) VALUES(?, ?)`, id, string(raw)); err != nil {
		return nil, err
	}
	return jsonResponse{body: item}, nil
}
