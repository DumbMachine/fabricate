...
func (*Resource) Descriptor() httpresource.Descriptor {
	...
	if err != nil {
		panic(fmt.Sprintf("mailchimp: embedded OpenAPI: %v", err))
	}
	raw, err := json.Marshal(spec)
	if err != nil {
		panic(fmt.Sprintf("mailchimp: marshal embedded OpenAPI: %v", err))
	}
	return httpresource.Descriptor{
		ID: "mailchimp", DisplayName: "Mailchimp", Version: "3.0", OpenAPIVersion: "2.0",
		OpenAPIDigest: httpresourceDigest(raw), ScenarioVersion: 1,
		ProviderHosts: []string{"us1.api.mailchimp.com"},
...
func (*Resource) Scenario(id string) (scenario.Document, error) {
	...
	if err != nil {
		return scenario.Document{}, fmt.Errorf("mailchimp: list embedded scenarios: %w", err)
	}
	for _, entry := range entries {
		raw, err := builtInScenarios.ReadFile("scenarios/" + entry.Name())
		if err != nil {
			return scenario.Document{}, fmt.Errorf("mailchimp: read embedded scenario %s: %w", entry.Name(), err)
		}
		doc, err := scenario.Parse(raw)
		if err != nil {
			return scenario.Document{}, fmt.Errorf("mailchimp: parse embedded scenario %s: %w", entry.Name(), err)
		}
		if doc.ID == id {
			return doc, nil
		}
	}
	return scenario.Document{}, fmt.Errorf("mailchimp: unknown scenario %q", id)
}
