...
const testToken = "fab_mailchimp_test_token"
...
func openTestDB(t *testing.T, doc scenario.Document) *sql.DB {
	t.Helper()
	db, err := sql.Open("sqlite", "file:"+filepath.Join(t.TempDir(), "mailchimp.db")+"?_pragma=foreign_keys(1)&_pragma=busy_timeout(5000)")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAcmeScenarioValidatesAndRoundTrips(t *testing.T) {
	doc := loadScenario(t, "acme-audience.v1.json")
	codec := scenarioCodec{}
...
func TestAcmeScenarioReadWriteRead(t *testing.T) {
	db := openTestDB(t, loadScenario(t, "acme-audience.v1.json"))
	handler := newTestHandler(t, db, &testIDs{})

	unauthorized := request(t, handler, http.MethodGet, "/3.0/ping", "", "")
	if unauthorized.Code != http.StatusUnauthorized {
		t.Fatalf("unauthorized = %d %s", unauthorized.Code, unauthorized.Body.String())
	}

	ping := request(t, handler, http.MethodGet, "/3.0/ping", "", testToken)
	if ping.Code != http.StatusOK || !strings.Contains(ping.Body.String(), "Chimpy") {
		t.Fatalf("ping = %d %s", ping.Code, ping.Body.String())
	}

	members := request(t, handler, http.MethodGet, "/3.0/lists/acme-customers/members", "", testToken)
	if members.Code != http.StatusOK || !strings.Contains(members.Body.String(), `"dana@northwind.example"`) {
		t.Fatalf("members = %d %s", members.Code, members.Body.String())
	}

	created := request(t, handler, http.MethodPost, "/3.0/lists/acme-customers/members", `{"email_address":"casey@acme.example","status":"subscribed","merge_fields":{"FNAME":"Casey","LNAME":"Pike"}}`, testToken)
	if created.Code != http.StatusOK || !strings.Contains(created.Body.String(), `"casey@acme.example"`) {
		t.Fatalf("add member = %d %s", created.Code, created.Body.String())
	}

	sent := request(t, handler, http.MethodPost, "/3.0/campaigns/campaign_inv4812/actions/send", "", testToken)
	if sent.Code != http.StatusNoContent {
		t.Fatalf("send campaign = %d %s", sent.Code, sent.Body.String())
	}

	campaign := request(t, handler, http.MethodGet, "/campaigns/campaign_inv4812", "", testToken)
	if campaign.Code != http.StatusOK || !strings.Contains(campaign.Body.String(), `"sent"`) {
		t.Fatalf("campaign after send = %d %s", campaign.Code, campaign.Body.String())
	}
}
