...
func TestAcmeScenarioReadWriteRead(t *testing.T) {
	...
	unauthorized := request(t, handler, http.MethodGet, "/v1/responses", "", "")
	if unauthorized.Code != http.StatusUnauthorized {
		t.Fatalf("unauthorized = %d %s", unauthorized.Code, unauthorized.Body.String())
	}

	listed := request(t, handler, http.MethodGet, "/v1/responses", "", testToken)
	...
	if !strings.Contains(created.Body.String(), `"id":"openai.response-0001"`) {
		t.Fatalf("create = %d %s", created.Code, created.Body.String())
	}

	persisted := request(t, handler, http.MethodGet, "/v1/responses/openai.response-0001", "", testToken)
