...
const testToken = "fab_openai_test_token"
...
func openTestDB(t *testing.T, doc scenario.Document) *sql.DB {
	t.Helper()
	db, err := sql.Open("sqlite", "file:"+filepath.Join(t.TempDir(), "openai.db")+"?_pragma=foreign_keys(1)&_pragma=busy_timeout(5000)")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAcmeScenarioValidatesAndRoundTrips(t *testing.T) {
	doc := loadScenario(t, "acme-responses.v1.json")
	codec := NewResource().Scenarios()
...
func TestCompiledOpenAPIContractIsValid(t *testing.T) {
	...
	if compiled.Index.Len() < 100 {
		t.Fatalf("compiled operation count = %d, want the full OpenAI catalog", compiled.Index.Len())
	}
...
func TestAcmeScenarioReadWriteRead(t *testing.T) {
	db := openTestDB(t, loadScenario(t, "acme-responses.v1.json"))
	handler := newTestHandler(t, db, &testIDs{})

	unauthorized := request(t, handler, http.MethodGet, "/v1/responses/resp_checkout", "", "")
	if unauthorized.Code != http.StatusUnauthorized {
		t.Fatalf("unauthorized = %d %s", unauthorized.Code, unauthorized.Body.String())
	}

	listed := request(t, handler, http.MethodGet, "/v1/responses/resp_checkout", "", testToken)
	...
	created := request(t, handler, http.MethodPost, "/v1/responses", `{"model":"gpt-4.1","input":"Summarize invoice INV-4812"}`, testToken)
	if created.Code != http.StatusOK && created.Code != http.StatusCreated {
		t.Fatalf("create = %d %s", created.Code, created.Body.String())
	}
	if !strings.Contains(created.Body.String(), `Summarize invoice INV-4812`) {
	...
	persisted := request(t, handler, http.MethodGet, strings.ReplaceAll("/v1/responses/{id}", "{id}", id), "", testToken)
	if persisted.Code != http.StatusOK || !strings.Contains(persisted.Body.String(), `Summarize invoice INV-4812`) {
...
func TestAllOperationsAreRouted(t *testing.T) {
	...
	minimal := "acme-responses.v1.json"
