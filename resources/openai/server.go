...
import (
	...
	"github.com/dumbmachine/fabricate/resources/openai/generated"
...
func (r jsonResponse) VisitListResponsesResponse(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitGetResponseResponse(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitCreateResponseResponse(w http.ResponseWriter) error {
	return r.write(w)
}
...
func newServer(ctx context.Context, dependencies httpresource.ServerDependencies) (*server, error) {
	if dependencies.DB == nil || dependencies.Clock == nil || dependencies.IDs == nil || dependencies.Secrets == nil {
		return nil, fmt.Errorf("openai: database, clock, ID generator, and secrets are required")
	}
	token, err := dependencies.Secrets.Get(ctx, "token")
	if err != nil {
		return nil, fmt.Errorf("openai: load synthetic token: %w", err)
	}
	if token == "" {
		return nil, fmt.Errorf("openai: synthetic token is empty")
	}
	spec, err := generated.GetSwagger()
	if err != nil {
		return nil, fmt.Errorf("openai: load OpenAPI: %w", err)
	}
...
func (s *server) ListResponses(ctx context.Context, _ generated.ListResponsesRequestObject) (generated.ListResponsesResponseObject, error) {
	items, err := listJSON(ctx, s.db, `SELECT body FROM responses ORDER BY id`)
	if err != nil {
		return nil, err
	}
...
func (s *server) GetResponse(ctx context.Context, request generated.GetResponseRequestObject) (generated.GetResponseResponseObject, error) {
	item, err := scanJSON(s.db.QueryRowContext(ctx, `SELECT body FROM responses WHERE id=?`, request.ResponseId))
...
func (s *server) CreateResponse(ctx context.Context, request generated.CreateResponseRequestObject) (generated.CreateResponseResponseObject, error) {
	item := bodyMap(ctx)
	if request.Body != nil {
		mergeMap(item, asMap(request.Body))
	}
	required, _ := item["model"].(string)
	if strings.TrimSpace(required) == "" {
		return jsonResponse{status: http.StatusBadRequest, body: map[string]any{"message": "model is required"}}, nil
	}
	id, err := s.ids.Next(ctx, "openai.response")
	if err != nil {
		return nil, err
	}
	item["id"] = id
	...
	if _, err := s.db.ExecContext(ctx, `INSERT INTO responses(id, body) VALUES(?, ?)`, id, string(raw)); err != nil {
		return nil, err
	}
	return jsonResponse{body: item}, nil
}
