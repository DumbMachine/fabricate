...
import (
	...
	"github.com/dumbmachine/fabricate/resources/cloudflare/generated"
...
func (r jsonResponse) VisitListZonesResponse(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitGetZoneResponse(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitCreateZoneResponse(w http.ResponseWriter) error {
	return r.write(w)
}
...
func writeError(w http.ResponseWriter, status int, message string) {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(status)
	_, _ = w.Write([]byte(fmt.Sprintf(`{"success":false,"errors":[{"message":%q}]}`, message)))
}
...
func newServer(ctx context.Context, dependencies httpresource.ServerDependencies) (*server, error) {
	if dependencies.DB == nil || dependencies.Clock == nil || dependencies.IDs == nil || dependencies.Secrets == nil {
		return nil, fmt.Errorf("cloudflare: database, clock, ID generator, and secrets are required")
	}
	token, err := dependencies.Secrets.Get(ctx, "token")
	if err != nil {
		return nil, fmt.Errorf("cloudflare: load synthetic token: %w", err)
	}
	if token == "" {
		return nil, fmt.Errorf("cloudflare: synthetic token is empty")
	}
	spec, err := generated.GetSwagger()
	if err != nil {
		return nil, fmt.Errorf("cloudflare: load OpenAPI: %w", err)
	}
...
func (s *server) ListZones(ctx context.Context, _ generated.ListZonesRequestObject) (generated.ListZonesResponseObject, error) {
	items, err := listJSON(ctx, s.db, `SELECT body FROM zones ORDER BY id`)
	if err != nil {
		return nil, err
	}
	return jsonResponse{body: map[string]any{
		...
		"messages":    []any{},
		"result":      items,
		"result_info": map[string]any{"count": len(items), "page": 1, "per_page": 50, "total_count": len(items)},
	}}, nil
}

func (s *server) GetZone(ctx context.Context, request generated.GetZoneRequestObject) (generated.GetZoneResponseObject, error) {
	item, err := scanJSON(s.db.QueryRowContext(ctx, `SELECT body FROM zones WHERE id=?`, request.ZoneId))
	if errors.Is(err, sql.ErrNoRows) {
		return jsonResponse{status: http.StatusNotFound, body: map[string]any{"success": false, "errors": []any{map[string]any{"message": "Zone not found"}}, "result": nil}}, nil
	}
	if err != nil {
		return nil, err
	}
...
func (s *server) CreateZone(ctx context.Context, request generated.CreateZoneRequestObject) (generated.CreateZoneResponseObject, error) {
	item := bodyMap(ctx)
	if request.Body != nil {
	...
	id, err := s.ids.Next(ctx, "cloudflare.zone")
	if err != nil {
		return nil, err
	}
	item["id"] = id
	...
	if _, err := s.db.ExecContext(ctx, `INSERT INTO zones(id, body) VALUES(?, ?)`, id, string(raw)); err != nil {
		return nil, err
	}
	return jsonResponse{body: map[string]any{
		"success":  true,
