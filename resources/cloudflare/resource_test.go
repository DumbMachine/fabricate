...
func TestAcmeScenarioReadWriteRead(t *testing.T) {
	...
	if !strings.Contains(created.Body.String(), `"id":"cloudflare.zone-0001"`) {
		t.Fatalf("create = %d %s", created.Code, created.Body.String())
	}

	persisted := request(t, handler, http.MethodGet, "/client/v4/zones/cloudflare.zone-0001", "", testToken)
