...
func (*Resource) Descriptor() httpresource.Descriptor {
	...
	if err != nil {
		panic(fmt.Sprintf("cloudflare: embedded OpenAPI: %v", err))
	}
	raw, err := json.Marshal(spec)
	if err != nil {
		panic(fmt.Sprintf("cloudflare: marshal embedded OpenAPI: %v", err))
	}
	return httpresource.Descriptor{
		ID: "cloudflare", DisplayName: "Cloudflare", Version: "v1", OpenAPIVersion: "3.0.3",
		OpenAPIDigest: httpresourceDigest(raw), ScenarioVersion: 1,
		ProviderHosts: []string{"api.cloudflare.com"},
...
func (*Resource) Scenario(id string) (scenario.Document, error) {
	...
	if err != nil {
		return scenario.Document{}, fmt.Errorf("cloudflare: list embedded scenarios: %w", err)
	}
	for _, entry := range entries {
		raw, err := builtInScenarios.ReadFile("scenarios/" + entry.Name())
		if err != nil {
			return scenario.Document{}, fmt.Errorf("cloudflare: read embedded scenario %s: %w", entry.Name(), err)
		}
		doc, err := scenario.Parse(raw)
		if err != nil {
			return scenario.Document{}, fmt.Errorf("cloudflare: parse embedded scenario %s: %w", entry.Name(), err)
		}
		if doc.ID == id {
			return doc, nil
		}
	}
	return scenario.Document{}, fmt.Errorf("cloudflare: unknown scenario %q", id)
}
