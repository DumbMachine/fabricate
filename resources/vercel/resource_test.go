...
func TestAcmeScenarioReadWriteRead(t *testing.T) {
	...
	byName := request(t, handler, http.MethodGet, "/v9/projects/acme-api", "", testToken)
	if byName.Code != http.StatusOK || !strings.Contains(byName.Body.String(), `"prj_acme_api"`) {
		t.Fatalf("get project by name = %d %s", byName.Code, byName.Body.String())
	}

	created := request(t, handler, http.MethodPost, "/v11/projects", `{"name":"acme-billing"}`, testToken)
	if created.Code != http.StatusOK || !strings.Contains(created.Body.String(), `"id":"vercel.project-0001"`) {
		t.Fatalf("create = %d %s", created.Code, created.Body.String())
	}

	persisted := request(t, handler, http.MethodGet, "/v9/projects/vercel.project-0001", "", testToken)
