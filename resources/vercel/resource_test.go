...
const testToken = "fab_vercel_test_token"
...
func openTestDB(t *testing.T, doc scenario.Document) *sql.DB {
	t.Helper()
	db, err := sql.Open("sqlite", "file:"+filepath.Join(t.TempDir(), "vercel.db")+"?_pragma=foreign_keys(1)&_pragma=busy_timeout(5000)")
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompiledOpenAPIContractIsValid(t *testing.T) {
	...
	if compiled.Index.Len() < 100 {
		t.Fatalf("compiled operation count = %d, want the full Vercel catalog", compiled.Index.Len())
	}
...
func TestAcmeScenarioReadWriteRead(t *testing.T) {
	...
	unauthorized := request(t, handler, http.MethodGet, "/v10/projects", "", "")
	if unauthorized.Code != http.StatusUnauthorized {
		t.Fatalf("unauthorized = %d %s", unauthorized.Code, unauthorized.Body.String())
	}

	listed := request(t, handler, http.MethodGet, "/v10/projects", "", testToken)
	if listed.Code != http.StatusOK || !strings.Contains(listed.Body.String(), `acme-web`) {
		t.Fatalf("list = %d %s", listed.Code, listed.Body.String())
	}
	if !strings.Contains(listed.Body.String(), `acme-changelog`) {
		t.Fatalf("haystack missing acme-changelog: %s", listed.Body.String())
	}
	if strings.Count(listed.Body.String(), `"id":`) < 15 {
		t.Fatalf("project haystack too small: %s", listed.Body.String())
	}

	got := request(t, handler, http.MethodGet, "/v9/projects/prj_acme_web", "", testToken)
	if got.Code != http.StatusOK || !strings.Contains(got.Body.String(), `acme-web`) {
		t.Fatalf("get = %d %s", got.Code, got.Body.String())
	}

	created := request(t, handler, http.MethodPost, "/v11/projects", `{"name":"acme-billing"}`, testToken)
	...
	persisted := request(t, handler, http.MethodGet, strings.ReplaceAll("/v9/projects/{id}", "{id}", id), "", testToken)
