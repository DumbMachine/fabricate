...
token=${FAB_VERCEL_TOKEN:-}
if [ -z "$token" ]; then
  echo "FAB_VERCEL_TOKEN is required" >&2
  exit 1
fi

if [ "$mode" = "direct" ]; then
  if [ -z "${FAB_VERCEL_URL:-}" ]; then
    echo "FAB_VERCEL_URL is required in direct mode" >&2
    exit 1
  fi
  base=${FAB_VERCEL_URL%/}
  curl_opts=(-sS --fail)
...
listed=$(read_json "$base/v10/projects")
echo "$listed" | jq -e '.. | strings | select(. == "acme-web")' >/dev/null

got=$(read_json "$base/v9/projects/prj_acme_web")
echo "$got" | jq -e '.. | strings | select(. == "acme-web")' >/dev/null

created=$(read_json -X POST "$base/v11/projects" -d '{"name":"acme-billing"}')
created_id=$(echo "$created" | jq -r '.id')
test -n "$created_id"

persisted=$(read_json "$base/v9/projects/$created_id")
...
if path:
    report = {
        ...
        "environment": {
            "label": "Acme Vercel",
            "manifest": "environments/acme-vercel.yaml",
            "messages": 3,
        },
        "integration": "vercel",
        "modes": {},
