...
func (*Resource) Descriptor() httpresource.Descriptor {
	...
	if err != nil {
		panic(fmt.Sprintf("vercel: embedded OpenAPI: %v", err))
	}
	raw, err := json.Marshal(spec)
	if err != nil {
		panic(fmt.Sprintf("vercel: marshal embedded OpenAPI: %v", err))
	}
	return httpresource.Descriptor{
		ID: "vercel", DisplayName: "Vercel", Version: "v1", OpenAPIVersion: "3.0.3",
		OpenAPIDigest: httpresourceDigest(raw), ScenarioVersion: 1,
		ProviderHosts: []string{"api.vercel.com"},
...
func (*Resource) Scenario(id string) (scenario.Document, error) {
	...
	if err != nil {
		return scenario.Document{}, fmt.Errorf("vercel: list embedded scenarios: %w", err)
	}
	for _, entry := range entries {
		raw, err := builtInScenarios.ReadFile("scenarios/" + entry.Name())
		if err != nil {
			return scenario.Document{}, fmt.Errorf("vercel: read embedded scenario %s: %w", entry.Name(), err)
		}
		doc, err := scenario.Parse(raw)
		if err != nil {
			return scenario.Document{}, fmt.Errorf("vercel: parse embedded scenario %s: %w", entry.Name(), err)
		}
		if doc.ID == id {
			return doc, nil
		}
	}
	return scenario.Document{}, fmt.Errorf("vercel: unknown scenario %q", id)
}
