...
func (r jsonResponse) VisitGetTeamWebhooksResponse(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitGetWebhookResponse(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitPostWebhookResponse(w http.ResponseWriter) error {
	return r.write(w)
}
...
func newServer(ctx context.Context, dependencies httpresource.ServerDependencies) (*server, error) {
	if dependencies.DB == nil || dependencies.Clock == nil || dependencies.IDs == nil || dependencies.Secrets == nil {
		return nil, fmt.Errorf("figma: database, clock, ID generator, and secrets are required")
	}
	token, err := dependencies.Secrets.Get(ctx, "token")
	if err != nil {
		return nil, fmt.Errorf("figma: load synthetic token: %w", err)
	}
	if token == "" {
		return nil, fmt.Errorf("figma: synthetic token is empty")
	}
	spec, err := generated.GetSwagger()
	if err != nil {
		return nil, fmt.Errorf("figma: load OpenAPI: %w", err)
	}
...
func (s *server) GetTeamWebhooks(ctx context.Context, _ generated.GetTeamWebhooksRequestObject) (generated.GetTeamWebhooksResponseObject, error) {
	items, err := listJSON(ctx, s.db, `SELECT body FROM webhooks ORDER BY id`)
	if err != nil {
		return nil, err
	}
	return jsonResponse{body: map[string]any{"webhooks": items}}, nil
}

func (s *server) GetWebhook(ctx context.Context, request generated.GetWebhookRequestObject) (generated.GetWebhookResponseObject, error) {
	item, err := scanJSON(s.db.QueryRowContext(ctx, `SELECT body FROM webhooks WHERE id=?`, request.WebhookId))
...
func (s *server) PostWebhook(ctx context.Context, request generated.PostWebhookRequestObject) (generated.PostWebhookResponseObject, error) {
	item := bodyMap(ctx)
	if request.Body != nil {
		mergeMap(item, asMap(request.Body))
	}
	required, _ := item["endpoint"].(string)
	if strings.TrimSpace(required) == "" {
		return jsonResponse{status: http.StatusBadRequest, body: map[string]any{"message": "endpoint is required"}}, nil
	}
	id, err := s.ids.Next(ctx, "figma.webhook")
	if err != nil {
		return nil, err
	}
	item["id"] = id
	...
	if _, err := s.db.ExecContext(ctx, `INSERT INTO webhooks(id, body) VALUES(?, ?)`, id, string(raw)); err != nil {
		return nil, err
	}
	return jsonResponse{body: item}, nil
}
