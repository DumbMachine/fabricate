...
const testToken = "fab_figma_test_token"
...
func openTestDB(t *testing.T, doc scenario.Document) *sql.DB {
	t.Helper()
	db, err := sql.Open("sqlite", "file:"+filepath.Join(t.TempDir(), "figma.db")+"?_pragma=foreign_keys(1)&_pragma=busy_timeout(5000)")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAcmeScenarioReadWriteRead(t *testing.T) {
	...
	if !strings.Contains(listed.Body.String(), `https://acme.example/hooks/blog`) {
		t.Fatalf("haystack missing blog webhook: %s", listed.Body.String())
	}
	if strings.Count(listed.Body.String(), `"id":`) < 15 {
		t.Fatalf("webhook haystack too small: %s", listed.Body.String())
	}
