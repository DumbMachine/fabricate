...
func (r jsonResponse) VisitListProjectsResponse(w http.ResponseWriter) error {
	return r.write(w)
}
...
func newServer(ctx context.Context, dependencies httpresource.ServerDependencies) (*server, error) {
	if dependencies.DB == nil || dependencies.Clock == nil || dependencies.IDs == nil || dependencies.Secrets == nil {
		return nil, fmt.Errorf("neon: database, clock, ID generator, and secrets are required")
	}
	token, err := dependencies.Secrets.Get(ctx, "token")
	if err != nil {
		return nil, fmt.Errorf("neon: load synthetic token: %w", err)
	}
	if token == "" {
		return nil, fmt.Errorf("neon: synthetic token is empty")
	}
	spec, err := generated.GetSwagger()
	if err != nil {
		return nil, fmt.Errorf("neon: load OpenAPI: %w", err)
	}
...
func (s *server) ListProjects(ctx context.Context, _ generated.ListProjectsRequestObject) (generated.ListProjectsResponseObject, error) {
	items, err := listJSON(ctx, s.db, `SELECT body FROM projects ORDER BY id`)
	if err != nil {
		return nil, err
	}
	return jsonResponse{body: map[string]any{"projects": items}}, nil
}
...
func (s *server) GetProject(ctx context.Context, request generated.GetProjectRequestObject) (generated.GetProjectResponseObject, error) {
	item, err := scanJSON(s.db.QueryRowContext(ctx, `SELECT body FROM projects WHERE id=?`, request.ProjectId))
	...
	return jsonResponse{body: map[string]any{"project": item}}, nil
}
...
func (s *server) CreateProject(ctx context.Context, request generated.CreateProjectRequestObject) (generated.CreateProjectResponseObject, error) {
	...
	if nested, ok := item["project"].(map[string]any); ok {
		item = nested
	}
	...
	id, err := s.ids.Next(ctx, "neon.project")
	if err != nil {
		return nil, err
	}
	item["id"] = id
	if _, ok := item["region_id"]; !ok {
		item["region_id"] = "aws-us-east-2"
	}
	if _, ok := item["pg_version"]; !ok {
		item["pg_version"] = 16
	}
	...
	if _, ok := item["updated_at"]; !ok {
		item["updated_at"] = s.clock.Now().UTC().Format(time.RFC3339)
	}
