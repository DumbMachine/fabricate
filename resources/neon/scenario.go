...
var compiledScenarioSchema = func() *jsonschema.Schema {
	...
	if err != nil {
		panic(fmt.Sprintf("neon: parse embedded scenario schema: %v", err))
	}
	compiler := jsonschema.NewCompiler()
	if err := compiler.AddResource("neon-scenario.json", parsed); err != nil {
		panic(fmt.Sprintf("neon: add embedded scenario schema: %v", err))
	}
	compiled, err := compiler.Compile("neon-scenario.json")
	if err != nil {
		panic(fmt.Sprintf("neon: compile embedded scenario schema: %v", err))
	}
	return compiled
}()
...
func (scenarioCodec) Validate(_ context.Context, doc scenario.Document) error {
	...
	if doc.Resource != "neon" || doc.ResourceVersion != "v1" {
		return fmt.Errorf("neon scenario: expected resource neon v1, got %s %s", doc.Resource, doc.ResourceVersion)
	}
	instance, err := jsonschema.UnmarshalJSON(bytes.NewReader(doc.State))
	if err != nil {
		return fmt.Errorf("neon scenario: decode state for schema validation: %w", err)
	}
	if err := compiledScenarioSchema.Validate(instance); err != nil {
		return fmt.Errorf("neon scenario: schema validation: %w", err)
	}
...
func (scenarioCodec) Initialize(ctx context.Context, db *sql.DB) error {
	if _, err := db.ExecContext(ctx, schemaSQL); err != nil {
		return fmt.Errorf("neon scenario: initialize: %w", err)
	}
	return nil
}
...
func (codec scenarioCodec) Load(ctx context.Context, db *sql.DB, doc scenario.Document) error {
	...
	if err != nil {
		return fmt.Errorf("neon scenario: begin load: %w", err)
	}
	defer tx.Rollback()
	if _, err := tx.ExecContext(ctx, "DELETE FROM projects"); err != nil {
		return fmt.Errorf("neon scenario: clear projects: %w", err)
	}
	for _, item := range state.Projects {
		...
		if _, err := tx.ExecContext(ctx, `INSERT INTO projects(id, body) VALUES(?, ?)`, id, string(raw)); err != nil {
			return fmt.Errorf("neon scenario: insert projects %s: %w", id, err)
		}
	}
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("neon scenario: commit load: %w", err)
	}
	return nil
}
...
func (scenarioCodec) Dump(ctx context.Context, db *sql.DB, metadata scenario.Metadata) (scenario.Document, error) {
	...
	if err != nil {
		return scenario.Document{}, fmt.Errorf("neon scenario: dump projects: %w", err)
	}
	defer rows.Close()
	for rows.Next() {
		var id, raw string
	...
	return scenario.Document{
		...
		Resource: "neon", ResourceVersion: "v1", State: raw,
	}, nil
}
...
func decodeState(raw []byte) (fixtureState, error) {
	...
	if err := dec.Decode(&state); err != nil {
		return fixtureState{}, fmt.Errorf("neon scenario: decode state: %w", err)
	}
	var extra any
	if err := dec.Decode(&extra); err != io.EOF {
		return fixtureState{}, fmt.Errorf("neon scenario: state has trailing data")
	}
	if state.Projects == nil {
		return fixtureState{}, fmt.Errorf("neon scenario: required arrays are missing")
	}
	return state, nil
}
