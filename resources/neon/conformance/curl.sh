...
token=${FAB_NEON_TOKEN:-}
if [ -z "$token" ]; then
  echo "FAB_NEON_TOKEN is required" >&2
  exit 1
fi

if [ "$mode" = "direct" ]; then
  if [ -z "${FAB_NEON_URL:-}" ]; then
    echo "FAB_NEON_URL is required in direct mode" >&2
    exit 1
  fi
  base=${FAB_NEON_URL%/}
  curl_opts=(-sS --fail)
else
  base="https://console.neon.tech"
...
listed=$(read_json "$base/api/v2/projects")
echo "$listed" | jq -e '.. | strings | select(. == "acme-prod")' >/dev/null

got=$(read_json "$base/api/v2/projects/acme-prod")
echo "$got" | jq -e '.. | strings | select(. == "acme-prod")' >/dev/null

created=$(read_json -X POST "$base/api/v2/projects" -d '{"project":{"name":"acme-billing","region_id":"aws-us-east-2"}}')
created_id=$(echo "$created" | jq -r '.project.id')
test -n "$created_id" && test "$created_id" != "null"

persisted=$(read_json "$base/api/v2/projects/$created_id")
...
if path:
    report = {
        ...
        "environment": {
            "label": "Acme Neon",
            "manifest": "environments/acme-neon.yaml",
            "messages": 3,
        },
        "integration": "neon",
        "modes": {},
