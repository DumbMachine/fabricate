...
const testToken = "fab_neon_test_token"
...
func openTestDB(t *testing.T, doc scenario.Document) *sql.DB {
	t.Helper()
	db, err := sql.Open("sqlite", "file:"+filepath.Join(t.TempDir(), "neon.db")+"?_pragma=foreign_keys(1)&_pragma=busy_timeout(5000)")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAcmeScenarioValidatesAndRoundTrips(t *testing.T) {
	doc := loadScenario(t, "acme-projects.v1.json")
	codec := NewResource().Scenarios()
...
func TestCompiledOpenAPIContractIsValid(t *testing.T) {
	...
	if compiled.Index.Len() < 50 {
		t.Fatalf("compiled operation count = %d, want the full Neon catalog", compiled.Index.Len())
	}
...
func TestAcmeScenarioReadWriteRead(t *testing.T) {
	db := openTestDB(t, loadScenario(t, "acme-projects.v1.json"))
	handler := newTestHandler(t, db, &testIDs{})

	unauthorized := request(t, handler, http.MethodGet, "/api/v2/projects", "", "")
	if unauthorized.Code != http.StatusUnauthorized {
		t.Fatalf("unauthorized = %d %s", unauthorized.Code, unauthorized.Body.String())
	}

	listed := request(t, handler, http.MethodGet, "/api/v2/projects", "", testToken)
	if listed.Code != http.StatusOK || !strings.Contains(listed.Body.String(), `acme-prod`) {
		t.Fatalf("list = %d %s", listed.Code, listed.Body.String())
	}
	if !strings.Contains(listed.Body.String(), `acme-warehouse`) {
		t.Fatalf("haystack missing acme-warehouse: %s", listed.Body.String())
	}
	if strings.Count(listed.Body.String(), `"id":`) < 10 {
		t.Fatalf("project haystack too small: %s", listed.Body.String())
	}

	got := request(t, handler, http.MethodGet, "/api/v2/projects/acme-prod", "", testToken)
	if got.Code != http.StatusOK || !strings.Contains(got.Body.String(), `acme-prod`) {
		t.Fatalf("get = %d %s", got.Code, got.Body.String())
	}

	created := request(t, handler, http.MethodPost, "/api/v2/projects", `{"project":{"name":"acme-billing","region_id":"aws-us-east-2"}}`, testToken)
	...
	persisted := request(t, handler, http.MethodGet, strings.ReplaceAll("/api/v2/projects/{id}", "{id}", id), "", testToken)
