...
token=${FAB_RESEND_TOKEN:-}
if [ -z "$token" ]; then
  echo "FAB_RESEND_TOKEN is required" >&2
  exit 1
fi

if [ "$mode" = "direct" ]; then
  if [ -z "${FAB_RESEND_URL:-}" ]; then
    echo "FAB_RESEND_URL is required in direct mode" >&2
    exit 1
  fi
  base=${FAB_RESEND_URL%/}
  curl_opts=(-sS --fail)
...
listed=$(read_json "$base/emails")
echo "$listed" | jq -e '.. | strings | select(. == "INV-4812 receipt")' >/dev/null

got=$(read_json "$base/emails/email_inv4812")
echo "$got" | jq -e '.. | strings | select(. == "dana@northwind.example")' >/dev/null

created=$(read_json -X POST "$base/emails" -d '{"from":"billing@acme.example","to":["priya@fernworks.example"],"subject":"CSV export"}')
created_id=$(echo "$created" | jq -r '.id')
test -n "$created_id"

persisted=$(read_json "$base/emails/$created_id")
echo "$persisted" | jq -e '.. | strings | select(. == "CSV export")' >/dev/null
...
if path:
    report = {
        ...
        "environment": {
            "label": "Acme Resend",
            "manifest": "environments/acme-resend.yaml",
            "messages": 3,
        },
        "integration": "resend",
        "modes": {},
        "operationLabels": {"read": "Read outbox", "write": "Send an email", "persistence": "Confirm persistence"},
        "verification": {
            "kind": "curl",
            "label": "HTTP client",
