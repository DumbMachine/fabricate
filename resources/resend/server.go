...
import (
	...
	"github.com/dumbmachine/fabricate/resources/resend/generated"
...
func (r jsonResponse) VisitDomainslistResponse(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitEmailslistResponse(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitEmailssendResponse(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitEmailsgetResponse(w http.ResponseWriter) error {
	return r.write(w)
}
...
func newServer(ctx context.Context, dependencies httpresource.ServerDependencies) (*server, error) {
	if dependencies.DB == nil || dependencies.Clock == nil || dependencies.IDs == nil || dependencies.Secrets == nil {
		return nil, fmt.Errorf("resend: database, clock, ID generator, and secrets are required")
	}
	token, err := dependencies.Secrets.Get(ctx, "token")
	if err != nil {
		return nil, fmt.Errorf("resend: load synthetic token: %w", err)
	}
	if token == "" {
		return nil, fmt.Errorf("resend: synthetic token is empty")
	}
	spec, err := generated.GetSwagger()
	if err != nil {
		return nil, fmt.Errorf("resend: load OpenAPI: %w", err)
	}
...
func (s *server) Domainslist(ctx context.Context, _ generated.DomainslistRequestObject) (generated.DomainslistResponseObject, error) {
	items, err := listJSON(ctx, s.db, `SELECT body FROM domains ORDER BY id`)
	if err != nil {
		return nil, err
	}
	return jsonResponse{body: map[string]any{"object": "list", "has_more": false, "data": items}}, nil
}

func (s *server) Emailslist(ctx context.Context, _ generated.EmailslistRequestObject) (generated.EmailslistResponseObject, error) {
	items, err := listJSON(ctx, s.db, `SELECT body FROM emails ORDER BY id`)
	if err != nil {
		return nil, err
	}
	return jsonResponse{body: map[string]any{"object": "list", "has_more": false, "data": items}}, nil
...
func (s *server) Emailssend(ctx context.Context, request generated.EmailssendRequestObject) (generated.EmailssendResponseObject, error) {
	item := bodyMap(ctx)
	if request.Body != nil {
		mergeMap(item, asMap(request.Body))
	}
	id, err := s.ids.Next(ctx, "resend.email")
	if err != nil {
		return nil, err
	}
	item["id"] = id
	...
	if _, ok := item["last_event"]; !ok {
		item["last_event"] = "delivered"
	}
	raw, err := json.Marshal(item)
	if err != nil {
		return nil, err
	}
	if _, err := s.db.ExecContext(ctx, `INSERT INTO emails(id, body) VALUES(?, ?)`, id, string(raw)); err != nil {
		return nil, err
	}
	return jsonResponse{body: map[string]any{"id": id}}, nil
}

func (s *server) Emailsget(ctx context.Context, request generated.EmailsgetRequestObject) (generated.EmailsgetResponseObject, error) {
	item, err := scanJSON(s.db.QueryRowContext(ctx, `SELECT body FROM emails WHERE id=?`, request.EmailId))
	if errors.Is(err, sql.ErrNoRows) {
		return jsonResponse{status: http.StatusNotFound, body: map[string]any{"name": "not_found", "message": "Email not found"}}, nil
	}
	if err != nil {
		return nil, err
	}
