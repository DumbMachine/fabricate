...
const testToken = "fab_resend_test_token"
...
func openTestDB(t *testing.T, doc scenario.Document) *sql.DB {
	t.Helper()
	db, err := sql.Open("sqlite", "file:"+filepath.Join(t.TempDir(), "resend.db")+"?_pragma=foreign_keys(1)&_pragma=busy_timeout(5000)")
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompiledOpenAPIContractIsValid(t *testing.T) {
	...
	if len(seen) != 4 {
		t.Fatalf("compiled operation count = %d, want 4", len(seen))
	}
...
func TestAcmeScenarioReadWriteRead(t *testing.T) {
	...
	unauthorized := request(t, handler, http.MethodGet, "/emails", "", "")
	if unauthorized.Code != http.StatusUnauthorized {
		t.Fatalf("unauthorized = %d %s", unauthorized.Code, unauthorized.Body.String())
	}

	listed := request(t, handler, http.MethodGet, "/emails", "", testToken)
	if listed.Code != http.StatusOK || !strings.Contains(listed.Body.String(), `"INV-4812 receipt"`) {
		t.Fatalf("list emails = %d %s", listed.Code, listed.Body.String())
	}

	got := request(t, handler, http.MethodGet, "/emails/email_inv4812", "", testToken)
	if got.Code != http.StatusOK || !strings.Contains(got.Body.String(), `"dana@northwind.example"`) {
		t.Fatalf("get email = %d %s", got.Code, got.Body.String())
	}

	created := request(t, handler, http.MethodPost, "/emails", `{"from":"billing@acme.example","to":["priya@fernworks.example"],"subject":"CSV export"}`, testToken)
	if created.Code != http.StatusOK || !strings.Contains(created.Body.String(), `"id":"resend.email-0001"`) {
		t.Fatalf("send = %d %s", created.Code, created.Body.String())
	}

	persisted := request(t, handler, http.MethodGet, "/emails/resend.email-0001", "", testToken)
	if persisted.Code != http.StatusOK || !strings.Contains(persisted.Body.String(), `"CSV export"`) {
		t.Fatalf("persisted send = %d %s", persisted.Code, persisted.Body.String())
	}

	domains := request(t, handler, http.MethodGet, "/domains", "", testToken)
	if domains.Code != http.StatusOK || !strings.Contains(domains.Body.String(), `"acme.example"`) {
