...
func (*Resource) Descriptor() httpresource.Descriptor {
	...
	if err != nil {
		panic(fmt.Sprintf("sendgrid: embedded OpenAPI: %v", err))
	}
	raw, err := json.Marshal(spec)
	if err != nil {
		panic(fmt.Sprintf("sendgrid: marshal embedded OpenAPI: %v", err))
	}
	return httpresource.Descriptor{
		ID: "sendgrid", DisplayName: "SendGrid", Version: "v3", OpenAPIVersion: "3.0.3",
		OpenAPIDigest: httpresourceDigest(raw), ScenarioVersion: 1,
		ProviderHosts: []string{"api.sendgrid.com"},
...
func (*Resource) Scenario(id string) (scenario.Document, error) {
	...
	if err != nil {
		return scenario.Document{}, fmt.Errorf("sendgrid: list embedded scenarios: %w", err)
	}
	for _, entry := range entries {
		raw, err := builtInScenarios.ReadFile("scenarios/" + entry.Name())
		if err != nil {
			return scenario.Document{}, fmt.Errorf("sendgrid: read embedded scenario %s: %w", entry.Name(), err)
		}
		doc, err := scenario.Parse(raw)
		if err != nil {
			return scenario.Document{}, fmt.Errorf("sendgrid: parse embedded scenario %s: %w", entry.Name(), err)
		}
		if doc.ID == id {
			return doc, nil
		}
	}
	return scenario.Document{}, fmt.Errorf("sendgrid: unknown scenario %q", id)
}
