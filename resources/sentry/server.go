...
import (
	...
	"github.com/dumbmachine/fabricate/resources/sentry/generated"
...
func (r jsonResponse) VisitListOrganizationProjectsResponse(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitGetProjectResponse(w http.ResponseWriter) error {
	return r.write(w)
}
func (r jsonResponse) VisitCreateOrganizationProjectResponse(w http.ResponseWriter) error {
	return r.write(w)
}
...
func newServer(ctx context.Context, dependencies httpresource.ServerDependencies) (*server, error) {
	if dependencies.DB == nil || dependencies.Clock == nil || dependencies.IDs == nil || dependencies.Secrets == nil {
		return nil, fmt.Errorf("sentry: database, clock, ID generator, and secrets are required")
	}
	token, err := dependencies.Secrets.Get(ctx, "token")
	if err != nil {
		return nil, fmt.Errorf("sentry: load synthetic token: %w", err)
	}
	if token == "" {
		return nil, fmt.Errorf("sentry: synthetic token is empty")
	}
	spec, err := generated.GetSwagger()
	if err != nil {
		return nil, fmt.Errorf("sentry: load OpenAPI: %w", err)
	}
...
func (s *server) ListOrganizationProjects(ctx context.Context, _ generated.ListOrganizationProjectsRequestObject) (generated.ListOrganizationProjectsResponseObject, error) {
...
func (s *server) GetProject(ctx context.Context, request generated.GetProjectRequestObject) (generated.GetProjectResponseObject, error) {
	item, err := scanJSON(s.db.QueryRowContext(ctx, `SELECT body FROM projects WHERE id=?`, request.ProjectIdOrSlug))
...
func (s *server) CreateOrganizationProject(ctx context.Context, request generated.CreateOrganizationProjectRequestObject) (generated.CreateOrganizationProjectResponseObject, error) {
	item := bodyMap(ctx)
	if request.Body != nil {
	...
	id, err := s.ids.Next(ctx, "sentry.project")
	if err != nil {
		return nil, err
	}
	item["id"] = id
