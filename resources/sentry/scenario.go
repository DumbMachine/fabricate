...
var compiledScenarioSchema = func() *jsonschema.Schema {
	...
	if err != nil {
		panic(fmt.Sprintf("sentry: parse embedded scenario schema: %v", err))
	}
	compiler := jsonschema.NewCompiler()
	if err := compiler.AddResource("sentry-scenario.json", parsed); err != nil {
		panic(fmt.Sprintf("sentry: add embedded scenario schema: %v", err))
	}
	compiled, err := compiler.Compile("sentry-scenario.json")
	if err != nil {
		panic(fmt.Sprintf("sentry: compile embedded scenario schema: %v", err))
	}
	return compiled
}()
...
func (scenarioCodec) Validate(_ context.Context, doc scenario.Document) error {
	...
	if doc.Resource != "sentry" || doc.ResourceVersion != "v1" {
		return fmt.Errorf("sentry scenario: expected resource sentry v1, got %s %s", doc.Resource, doc.ResourceVersion)
	}
	instance, err := jsonschema.UnmarshalJSON(bytes.NewReader(doc.State))
	if err != nil {
		return fmt.Errorf("sentry scenario: decode state for schema validation: %w", err)
	}
	if err := compiledScenarioSchema.Validate(instance); err != nil {
		return fmt.Errorf("sentry scenario: schema validation: %w", err)
	}
...
func (scenarioCodec) Initialize(ctx context.Context, db *sql.DB) error {
	if _, err := db.ExecContext(ctx, schemaSQL); err != nil {
		return fmt.Errorf("sentry scenario: initialize: %w", err)
	}
	return nil
}
...
func (codec scenarioCodec) Load(ctx context.Context, db *sql.DB, doc scenario.Document) error {
	...
	if err != nil {
		return fmt.Errorf("sentry scenario: begin load: %w", err)
	}
	defer tx.Rollback()
	if _, err := tx.ExecContext(ctx, "DELETE FROM projects"); err != nil {
		return fmt.Errorf("sentry scenario: clear projects: %w", err)
	}
	for _, item := range state.Projects {
		...
		if _, err := tx.ExecContext(ctx, `INSERT INTO projects(id, body) VALUES(?, ?)`, id, string(raw)); err != nil {
			return fmt.Errorf("sentry scenario: insert projects %s: %w", id, err)
		}
	}
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("sentry scenario: commit load: %w", err)
	}
	return nil
}
...
func (scenarioCodec) Dump(ctx context.Context, db *sql.DB, metadata scenario.Metadata) (scenario.Document, error) {
	...
	if err != nil {
		return scenario.Document{}, fmt.Errorf("sentry scenario: dump projects: %w", err)
	}
	defer rows.Close()
	for rows.Next() {
		var id, raw string
	...
	return scenario.Document{
		...
		Resource: "sentry", ResourceVersion: "v1", State: raw,
	}, nil
}
...
func decodeState(raw []byte) (fixtureState, error) {
	...
	if err := dec.Decode(&state); err != nil {
		return fixtureState{}, fmt.Errorf("sentry scenario: decode state: %w", err)
	}
	var extra any
	if err := dec.Decode(&extra); err != io.EOF {
		return fixtureState{}, fmt.Errorf("sentry scenario: state has trailing data")
	}
	if state.Projects == nil {
		return fixtureState{}, fmt.Errorf("sentry scenario: required arrays are missing")
	}
	return state, nil
}
