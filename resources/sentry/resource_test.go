...
const testToken = "fab_sentry_test_token"
...
func openTestDB(t *testing.T, doc scenario.Document) *sql.DB {
	t.Helper()
	db, err := sql.Open("sqlite", "file:"+filepath.Join(t.TempDir(), "sentry.db")+"?_pragma=foreign_keys(1)&_pragma=busy_timeout(5000)")
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompiledOpenAPIContractIsValid(t *testing.T) {
	...
	if compiled.Index.Len() < 50 {
		t.Fatalf("compiled operation count = %d, want the full Sentry catalog", compiled.Index.Len())
	}
...
func TestAcmeScenarioReadWriteRead(t *testing.T) {
	...
	unauthorized := request(t, handler, http.MethodGet, "/api/0/organizations/acme/projects/", "", "")
	if unauthorized.Code != http.StatusUnauthorized {
		t.Fatalf("unauthorized = %d %s", unauthorized.Code, unauthorized.Body.String())
	}

	listed := request(t, handler, http.MethodGet, "/api/0/organizations/acme/projects/", "", testToken)
	...
	created := request(t, handler, http.MethodPost, "/api/0/organizations/acme/projects/", `{"name":"acme-billing","platform":"python"}`, testToken)
	...
	persisted := request(t, handler, http.MethodGet, strings.ReplaceAll("/api/0/projects/acme/{id}/", "{id}", id), "", testToken)
