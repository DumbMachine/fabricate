...
const testToken = "fab_mailgun_test_token"
...
func openTestDB(t *testing.T, doc scenario.Document) *sql.DB {
	t.Helper()
	db, err := sql.Open("sqlite", "file:"+filepath.Join(t.TempDir(), "mailgun.db")+"?_pragma=foreign_keys(1)&_pragma=busy_timeout(5000)")
	if err != nil {
		t.Fatal(err)
	}
...
func requestForm(t *testing.T, handler http.Handler, method, path, body, token string) *httptest.ResponseRecorder {
	t.Helper()
	req := httptest.NewRequest(method, path, strings.NewReader(body))
...
func TestAcmeScenarioReadWriteRead(t *testing.T) {
	...
	unauthorized := request(t, handler, http.MethodGet, "/v4/domains", "", "")
	if unauthorized.Code != http.StatusUnauthorized {
		t.Fatalf("unauthorized = %d %s", unauthorized.Code, unauthorized.Body.String())
	}

	domains := request(t, handler, http.MethodGet, "/v4/domains", "", testToken)
	if domains.Code != http.StatusOK || !strings.Contains(domains.Body.String(), `"mg.acme.example"`) {
		t.Fatalf("domains = %d %s", domains.Code, domains.Body.String())
	}

	events := request(t, handler, http.MethodGet, "/v3/mg.acme.example/events", "", testToken)
	if events.Code != http.StatusOK || !strings.Contains(events.Body.String(), `"INV-4812 receipt"`) {
		t.Fatalf("events = %d %s", events.Code, events.Body.String())
	}

	sent := requestForm(t, handler, http.MethodPost, "/v3/mg.acme.example/messages",
		"from=billing@acme.example&to=priya@fernworks.example&subject=CSV+export&text=ready", testToken)
	if sent.Code != http.StatusOK || !strings.Contains(sent.Body.String(), `"Queued. Thank you."`) {
		t.Fatalf("send = %d %s", sent.Code, sent.Body.String())
	}

	after := request(t, handler, http.MethodGet, "/v3/mg.acme.example/events", "", testToken)
	if after.Code != http.StatusOK || !strings.Contains(after.Body.String(), `"CSV export"`) {
		t.Fatalf("events after send = %d %s", after.Code, after.Body.String())
	}
}
