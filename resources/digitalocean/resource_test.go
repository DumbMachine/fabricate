...
const testToken = "fab_digitalocean_test_token"
...
func openTestDB(t *testing.T, doc scenario.Document) *sql.DB {
	t.Helper()
	db, err := sql.Open("sqlite", "file:"+filepath.Join(t.TempDir(), "digitalocean.db")+"?_pragma=foreign_keys(1)&_pragma=busy_timeout(5000)")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAcmeScenarioValidatesAndRoundTrips(t *testing.T) {
	doc := loadScenario(t, "acme-droplets.v1.json")
	codec := NewResource().Scenarios()
...
func TestCompiledOpenAPIContractIsValid(t *testing.T) {
	...
	if compiled.Index.Len() < 200 {
		t.Fatalf("compiled operation count = %d, want the full DigitalOcean catalog", compiled.Index.Len())
	}
...
func TestAcmeScenarioReadWriteRead(t *testing.T) {
	db := openTestDB(t, loadScenario(t, "acme-droplets.v1.json"))
	handler := newTestHandler(t, db, &testIDs{})

	unauthorized := request(t, handler, http.MethodGet, "/v2/droplets", "", "")
	if unauthorized.Code != http.StatusUnauthorized {
		t.Fatalf("unauthorized = %d %s", unauthorized.Code, unauthorized.Body.String())
	}

	listed := request(t, handler, http.MethodGet, "/v2/droplets", "", testToken)
	if listed.Code != http.StatusOK || !strings.Contains(listed.Body.String(), `acme-web`) {
		t.Fatalf("list = %d %s", listed.Code, listed.Body.String())
	}
	if !strings.Contains(listed.Body.String(), `acme-search`) {
		t.Fatalf("haystack missing acme-search: %s", listed.Body.String())
	}
	if strings.Count(listed.Body.String(), `"id":`) < 15 {
		t.Fatalf("droplet haystack too small: %s", listed.Body.String())
	}

	got := request(t, handler, http.MethodGet, "/v2/droplets/droplet_web", "", testToken)
	if got.Code != http.StatusOK || !strings.Contains(got.Body.String(), `acme-web`) {
		t.Fatalf("get = %d %s", got.Code, got.Body.String())
	}

	created := request(t, handler, http.MethodPost, "/v2/droplets", `{"name":"acme-billing","region":"nyc1","size":"s-1vcpu-1gb","image":"ubuntu-24-04-x64"}`, testToken)
	...
	persisted := request(t, handler, http.MethodGet, strings.ReplaceAll("/v2/droplets/{id}", "{id}", id), "", testToken)
